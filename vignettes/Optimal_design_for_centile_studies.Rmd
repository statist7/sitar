...
output:
  html_document:
    df_print: paged
vignette: |
  %\VignetteIndexEntry{Optimal design for growth reference centiles} %\VignetteEncoding{UTF-8} %\VignetteEngine{knitr::rmarkdown}
---
...
symmetry the 99.6th centile), with  `SEz` 0.033 compared to 0.065. 
...
accurately estimate the shape of the median curve soon after birth when it rises steeply.

## nagegp

The function `nagegp` extends `optimal_design` by giving the numbers of measurements to be
collected per age group, where the groups are of equal width.
...
its arguments `minage`, `maxage` and `n_groups` define the age range and number of groups,
 defaulting to 20 groups from 0 to 20 years, i.e. each one year width.
...
is fixed at `n`, and the target ages of measurement are given by `age_varying`.
...
```{r, tidy = TRUE}
knitr::kable(nagegp(z = 2, N = 3506, minage = 0, maxage = 5, n_groups = 5) %>% 
...
A third example contrasts the Cuban Growth Study design with the optimal design 
based on the specified precision of `SEz` = 0.053 on the 3rd centile (see earlier), 
...
The pale gray histogram shows the complex sample composition used in the Cuban Study, with
